...
describe("Test fetchStats", () => {
  it("should fetch correct stats", async () => {
    let stats = await fetchStats("ngeorger");
  ...
  it("should throw error", async () => {
    ...
    await expect(fetchStats("ngeorger")).rejects.toThrow(
  ...
  it("should fetch total commits", async () => {
    mock
      .onGet("https://api.github.com/search/commits?q=author:ngeorger")
      .reply(200, { total_count: 1000 });

    let stats = await fetchStats("ngeorger", true);
    const rank = calculateRank({
      all_commits: true,
      commits: 1000,
      prs: 300,
  ...
  it("should throw specific error when include_all_commits true and API returns error", async () => {
    ...
    expect(fetchStats("ngeorger", true)).rejects.toThrow(
  ...
  it("should exclude stars of the `test-repo-1` repository", async () => {
    ...
    let stats = await fetchStats("ngeorger", true, ["test-repo-1"]);
    const rank = calculateRank({
      all_commits: true,
      commits: 1000,
      prs: 300,
  ...
  it("should fetch additional stats when it requested", async () => {
    let stats = await fetchStats("ngeorger", false, [], true, true, true);
    const rank = calculateRank({
      all_commits: false,
      commits: 100,
      prs: 300,
